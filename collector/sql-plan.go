...
type sqlPlanCollector struct {
	descs [6]*prometheus.Desc
}

func init() {
	// registerCollector("sqlPlan", defaultEnabled, NewSQLPlanCollector)
}

// NewSQLPlanCollector returns a new Collector exposing session activity statistics.
func NewSQLPlanCollector() (Collector, error) {
	descs := [6]*prometheus.Desc{
	...
	return &sqlPlanCollector{descs}, nil
}

func (c *sqlPlanCollector) Update(db *sql.DB, ch chan<- prometheus.Metric) error {
	rows, err := db.Query(sqlPlanSQL)
